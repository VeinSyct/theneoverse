...
function CreateWhatsAppButtonAndWidget(options) {
	...
	const config = {
		// dev mode is for widget preview while generating the widget
		env: options.env || "",
    ...
    brandName: options.brandName || "Whatsapp",
			...
			<div class="df-btn-content">
				<div class="df-content-topbar">
					<div class="df-brand-img">
						<img src="${config.brandImgUrl}" height="100%" width="100%" style="border-radius: 50%;object-fit:cover" alt="img"/>
					</div>
				...
				<div class="df-content-bottombar">
					...
					<div class="df-bottombar-branding">
						...
						/>
						by
						<a href="https://theneoverse.web.app" target="_blank"> <i>#TheNeoVerse</i> </a>
					</div>
				</div>
			</div>
	...
	window.goToLink = () => {
		...
		if (config.env != "dev") {
      const redirectUrl = `https://wa.me/${config.linkid}?text=`+config.prefilledMsg
			window.open(redirectUrl);
		}
	}
	// By default open the widget on load in dev mode
	if(config.env == "dev") {
		dfToggle(true);
	}
	...
	window.checkWidgetSession = () => {
		const existingSession = window.localStorage.getItem("_wa_widget_session");
	...
	window.createWidgetSession = () => {
		...
		window.localStorage.setItem("_wa_widget_session", nextTimestamp);
	}
